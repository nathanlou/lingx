<template>
	<div class="titles">
		<div class="screen">
			...
			类型：<el-select v-model="style_value" placeholder="请选择" class="select">
				<el-option v-for="item in style_list" :key="item.value" :label="item.label" :value="item.value" />
			</el-select>
			SIM状态：<el-select v-model="status_value" placeholder="请选择" class="select">
				<el-option v-for="item in status_list" :key="item.value" :label="item.label" :value="item.value" />
			</el-select>
			提醒类别：<el-select v-model="remind" placeholder="请选择" class="select">
			...
			<span style="float: right;">
				<el-button class="btn" type="primary" icon="el-icon-edit" size="mini" @click='reminds()'>批量提醒</el-button>
				<el-button class="btn" type="primary" icon="el-icon-edit" size="mini"  @click='renews()'>批量续费</el-button>
			</span>
		</div>
		<div class="container_table">
			...
			<el-table ref="multipleTable" :data="tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)" border stripe
			 style="width: 100%;font-size: 13px;" :default-sort="{prop: 'date', order: 'descending'}" >
				...
				<el-table-column prop="sim" label="SIM卡号" align="center" width="150">
				...
				<el-table-column prop="simzt" label="SIM状态" align="center">
					<template slot-scope="scope">
						<el-tag :type="scope.row.simzt === '正常' ? 'primary' : 'danger'" disable-transitions>{{ scope.row.simzt }}</el-tag>
					</template>
				</el-table-column>
				<el-table-column prop="sbzt" label="设备状态" align="center">
					<template slot-scope="scope">
						<el-tag :type="scope.row.sbzt === '在线' ? 'primary' : 'danger'" disable-transitions>{{ scope.row.sbzt }}</el-tag>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="250" align="center">
					<template slot-scope="scope">
						<el-button class="btn" type="success" size="mini"  @click='remindsim(scope.$index, scope.row)'>提醒</el-button>
						<el-button class="btn" type="danger" size="mini" @click='renew(scope.$index, scope.row)'>续费</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<el-dialog title="续费" :visible.sync="xfdialog" width="40%">
			...
			<span slot="footer" class="dialog-footer">
				<el-button @click="xfdialog = false">取 消</el-button>
				<el-button type="primary" @click="xfdialog = false">确 定</el-button>
			</span>
		</el-dialog>
		<!-- 续费 -->
		<!-- 提醒 -->
		<el-dialog title="提醒" :visible.sync="txdialog" width="30%">
			<el-form ref="form" :model="form" label-width="90px">
			  <el-form-item label="企业名称">
			    {{row.company}}
			  </el-form-item>
			...
			<span slot="footer" class="dialog-footer">
				<el-button type="primary" @click="txdialog = false">发送通知</el-button>
			</span>
		</el-dialog>
		<!-- 提醒 -->
	</div>
